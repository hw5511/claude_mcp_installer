...
def get_raw_url(gist_id):
    """Gist의 raw URL을 API로 요청하여 가져옵니다."""
...
@mcp.tool()
async def html_to_notion(html: str, description: str = "HTML generated by Claude") -> dict:
    """HTML 코드를 GitHub Gist로 변환하고 Notion에서 임베드 가능한 URL을 반환합니다.
    
    Args:
        html: HTML 콘텐츠
        description: Gist 설명 (선택 사항)
    """
    if not html:
        return {
            "success": False,
    ...
    # 2. HTML 유효성 검사 및 최적화
    valid_html, validation_message = validate_and_optimize_html(html)
    if not valid_html:
        return {
            "success": False,
